...
/*
    ...
    off_t remainder;
    if(write){
        if(dir->attri.each_att.readonly) {
            free(dir);
            return -1;
    ...
    if (write) { // write
        ...
        if(offset + num_bytes < file_size ) {  //don't need to allocate new block 
            ...
            if( num_allocate > 0){
                if( alloc_block(fs, cluster_num, num_allocate) == -1 ) {
                    free(dir);
                    return -1;
                }
...
static int fat32_exists(void *state, char *path)
{
    dir_entry* dir_entry_ptr = malloc(sizeof(dir_entry));
    struct fat32_state *fs = (struct fat32_state *)state;
    //uint32_t dir_cluster;
    int rc = path_lookup(fs, path, NULL, dir_entry_ptr, 0) != -1;
    free(dir_entry_ptr);
    return rc;
}
...
static int fat32_truncate(void *state, void *file, off_t len)
{
    ...
    if(new_file_size_clusters < file_size_clusters) { //shrink
        ...
        nk_block_dev_read(fs->dev, get_sector_num(cluster_num, fs), 1, file_content, NK_DEV_REQ_BLOCKING);
        memset(file_content + size - 1, '\0', cluster_size - size + 1);
        nk_block_dev_write(fs->dev, get_sector_num(cluster_num, fs), 1, file_content, NK_DEV_REQ_BLOCKING);
        //alloc_block(fs, cluster_num, size_clusters_diff); 
        /*
    ...
    }else if(new_file_size_clusters > file_size_clusters) { //grow
        //grow
...
static void fat32_close(void *state, void *file)
{
    dir_entry * dir = malloc(sizeof(dir_entry));
    struct fat32_state *fs = (struct fat32_state *)state;
    if( path_lookup(fs, file, NULL, dir, 0) == -1 )
